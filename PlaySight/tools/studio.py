#!/usr/bin/env python3
"""
PlaySight Studio — a Cypress-style unified runner: functional + load tests,
...
def _ensure_dependencies():
    ...
    if os.environ.get("PLAYSIGHT_BOOTSTRAPPED") == "1":
    ...
    try:
        ...
        sys.exit(1)
    os.environ["PLAYSIGHT_BOOTSTRAPPED"] = "1"
...
app = FastAPI(title="PlaySight Studio", lifespan=lambda _app: _lifespan())
_runner = LoadRunner()
...
def _cmd_serve(args):
    ...
    print(f"\n🎭 PlaySight Studio → http://{args.host}:{args.port}")
...
def main():
    parser = argparse.ArgumentParser(description="PlaySight Studio — unified test runner")
...
<title>PlaySight — Load Runner</title>
...
<body>
<aside class="sb">
  <div class="brand"><div class="brand-logo">🎭 PlaySight</div><div class="brand-sub">Studio</div></div>
  <nav class="nav">
...
function downloadCompare(fmt){
  ...
  if(fmt==='json'){
    ...
    blob=new Blob([`<!doctype html><meta charset=utf-8><title>PlaySight — Run Comparison</title><h2>PlaySight — Run Comparison</h2><table style="border-collapse:collapse;font-family:system-ui"><thead><tr>${th}</tr></thead><tbody>${body}</tbody></table>`],{type:'text/html'}); name='comparison.html';
  }else{
