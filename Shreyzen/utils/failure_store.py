...
class FailureStore:
    ...
    def record(self, test_id: str, message: str = "", traceback: str = "",
               run_ts: str = "", browser: str = "") -> None:
        """Persist one failure. Never raises into the test run."""
        try:
            with self._conn() as conn:
                conn.execute(
                    "INSERT INTO test_failures "
                    "(test_id, run_ts, browser, message, traceback, recorded) "
                    "VALUES (?, ?, ?, ?, ?, ?)",
                    (test_id, run_ts, browser, (message or "")[:500],
                     (traceback or "")[:_MAX_TRACEBACK], datetime.now().isoformat()),
                )
    ...
    def recent(self, limit: int = 200) -> list:
        ...
        try:
            with self._conn() as conn:
                rows = conn.execute(
                    "SELECT test_id, run_ts, browser, message, traceback, recorded "
