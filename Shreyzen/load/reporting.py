...
matching the rest of the Shreyzen framework's reporting story.
...
def write_junit(run_dir: Path, meta: dict, verdict: dict) -> Path:
    ...
    testsuites = ET.Element("testsuites", {
        "name": "Shreyzen Load", "tests": str(len(endpoints)),
    ...
    for e in endpoints:
        case = ET.SubElement(testsuite, "testcase", {
            "name": e["name"],
            "classname": f"shreyzen.load.{meta.get('scenario', 'load')}",
            "time": f"{per:.2f}",
        })
        if not e["passed"]:
...
def write_allure(run_dir: Path, meta: dict, verdict: dict) -> Path:
    ...
    for e in verdict["endpoints"]:
        result = {
            ...
            "name": e["name"],
            "fullName": f"Shreyzen Load / {scenario} / {e['name']}",
...
def inject_context_panel(run_dir: Path, meta: dict) -> Optional[Path]:
    ...
    try:
        ...
        return None
    if "shreyzen-context" in html:  # already injected
        return report
    ...
    panel = (
        "<section class='shreyzen-context' style=\"font-family:system-ui,-apple-system,"
        "sans-serif;margin:16px;padding:16px 20px;border:1px solid #e2e8f0;border-radius:12px;"
        "background:#f8fafc\">"
        "<h3 style='margin:0 0 10px;font-size:14px;color:#1e293b'>🎯 Shreyzen — Run context "
