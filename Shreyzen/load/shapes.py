...
Env vars (all optional, sensible defaults):
    SHREYZEN_PROFILE      profile key: smoke|load|stress|spike|soak|breakpoint|custom
    SHREYZEN_USERS        peak virtual users (int)
    SHREYZEN_DURATION     total run length in seconds (int)
    SHREYZEN_SPAWN_RATE   VUs added/removed per second (float; 0 → auto)
"""
...
class ProfileShape(LoadTestShape):
    """Drive virtual users according to SHREYZEN_* environment configuration."""

    # Read once at import time — the engine sets these in the subprocess env.
    profile = os.getenv("SHREYZEN_PROFILE", "smoke")
    peak_users = _env_int("SHREYZEN_USERS", 3)
    duration = _env_int("SHREYZEN_DURATION", 30)
    spawn_rate = _env_float("SHREYZEN_SPAWN_RATE", 0.0)
